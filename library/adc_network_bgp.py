...
def adc_list_networks(module):
    ...
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.network.list" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取BGP网络列表失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取BGP网络列表失败", response=parsed_data)
            else:
...
def adc_add_network(module):
    ...
    ip = module.params['ip']
    authkey = module.params['authkey']
    network = module.params['network'] if 'network' in module.params else ""
    mask = module.params['mask'] if 'mask' in module.params else ""

    # 检查必需参数
    if not network or not mask:
        module.fail_json(msg="添加BGP网络需要提供network和mask参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.network.add" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="添加BGP网络失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "添加BGP网络", True)
        if success:
...
def adc_delete_network(module):
    ...
    # 检查必需参数
    if not network or not mask:
        module.fail_json(msg="删除BGP网络需要提供network和mask参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.network.del" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="删除BGP网络失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "删除BGP网络", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_list_neighbors(module):
    ...
    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.neighbor.list" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取BGP邻居列表失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取BGP邻居列表失败", response=parsed_data)
            else:
                module.exit_json(changed=False, neighbors=parsed_data)
        except Exception as e:
            module.fail_json(msg="解析响应失败: %s" % str(e))
    else:
        module.fail_json(msg="未收到有效响应")
...
def adc_add_neighbor(module):
    ...
    ip = module.params['ip']
    authkey = module.params['authkey']
    neighbor_ip = module.params['neighbor_ip'] if 'neighbor_ip' in module.params else ""
    remote_as = module.params['remote_as'] if 'remote_as' in module.params else ""

    # 检查必需参数
    if not neighbor_ip or not remote_as:
        module.fail_json(msg="添加BGP邻居需要提供neighbor_ip和remote_as参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.neighbor.add" % (
        ip, authkey)
    ...
    if 'password' in module.params and module.params['password'] is not None:
        neighbor_data['password'] = module.params['password']
    ...
    try:
        ...
        module.fail_json(msg="添加BGP邻居失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "添加BGP邻居", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_delete_neighbor(module):
    ...
    # 检查必需参数
    if not neighbor_ip:
        module.fail_json(msg="删除BGP邻居需要提供neighbor_ip参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.neighbor.del" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="删除BGP邻居失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "删除BGP邻居", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_get_status(module):
    ...
    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.status.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取BGP状态失败: %s" % str(e))

    # 对于获取状态操作，直接返回响应数据，不判断success
    if response_data:
        try:
            parsed_data = json.loads(response_data)
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取BGP状态失败", response=parsed_data)
            else:
                module.exit_json(changed=False, status=parsed_data)
        except Exception as e:
            module.fail_json(msg="解析响应失败: %s" % str(e))
    else:
        module.fail_json(msg="未收到有效响应")
...
def adc_set_status(module):
    ...
    # 检查必需参数
    if enable == "":
        module.fail_json(msg="设置BGP状态需要提供enable参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=bgp.status.set" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="设置BGP状态失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置BGP状态", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def main():
    ...
    module_args = dict(
        ...
        neighbor_ip=dict(type='str', required=False),
        remote_as=dict(type='int', required=False),
