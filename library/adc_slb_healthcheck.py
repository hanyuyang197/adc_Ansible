...
def adc_healthcheck_list_withcommon(module):
    """获取 common 和分区健康检查列表"""
    ip = module.params['ip']
    authkey = module.params['authkey']

    url = "http://%s/adcapi/v2.0/?authkey=%s&action=slb.healthcheck.list.withcommon" % (
        ip, authkey)
    ...
    try:
        ...
        # 直接返回响应数据，不解析为特定格式
        module.exit_json(
            chchanged=False, healthcheck_list_withcommon=response_data)
    except Exception as e:
        module.fail_json(msg="获取 common 和分区健康检查列表失败: %s" % str(e))
...
def main():
    ...
    if action == 'healthcheck_list_withcommon':
        adc_healthcheck_list_withcommon(module)
