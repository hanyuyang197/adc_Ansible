...
def route_static_ctrl_ipv4_list(module):
    ...
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=route.static.ctrl.ipv4.list" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取IPv4静态控制路由列表失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取IPv4静态控制路由列表失败", response=parsed_data)
            else:
...
def route_static_ctrl_ipv4_get(module):
    ...
    if not destination or not netmask or not gateway:
        module.fail_json(msg="获取IPv4静态控制路由详情需要提供destination、netmask和gateway参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=route.static.ctrl.ipv4.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取IPv4静态控制路由详情失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取IPv4静态控制路由详情失败", response=parsed_data)
            else:
...
def route_static_ctrl_ipv4_add(module):
    ...
    # 检查必需参数
    if not destination or not netmask or not gateway:
        module.fail_json(msg="添加IPv4静态控制路由需要提供destination、netmask和gateway参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=route.static.ctrl.ipv4.add" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="添加IPv4静态控制路由失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "添加IPv4静态控制路由", True)
        if success:
...
def route_static_ctrl_ipv4_edit(module):
    ...
    # 检查必需参数
    if not destination or not netmask or not gateway:
        module.fail_json(msg="编辑IPv4静态控制路由需要提供destination、netmask和gateway参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=route.static.ctrl.ipv4.edit" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="编辑IPv4静态控制路由失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "编辑IPv4静态控制路由", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def route_static_ctrl_ipv4_del(module):
    ...
    # 检查必需参数
    if not destination or not netmask or not gateway:
        module.fail_json(msg="删除IPv4静态控制路由需要提供destination、netmask和gateway参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=route.static.ctrl.ipv4.del" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="删除IPv4静态控制路由失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "删除IPv4静态控制路由", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
