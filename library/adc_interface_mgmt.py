...
def adc_interface_mgmt_set(module):
    """管理口配置设置"""
    ip = module.params['ip']
    authkey = module.params['authkey']

    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=interface.mgmt.set" % (ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="管理口配置设置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "管理口配置设置", True)
        if success:
