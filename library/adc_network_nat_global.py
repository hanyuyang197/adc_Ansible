...
def nat_global_get(module):
    """获取NAT全局配置"""
    ip = module.params['ip']
    authkey = module.params['authkey']

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.global.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取NAT全局配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取NAT全局配置失败", response=parsed_data)
            else:
                module.exit_json(changed=False, global_config=parsed_data)
        except Exception as e:
...
def nat_global_set(module):
    ...
    try:
        ...
        module.fail_json(msg="设置NAT全局配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置NAT全局配置", True)
        if success:
...
def main():
    ...
    if action == 'nat_global_get':
        nat_global_get(module)
    elif action == 'nat_global_set':
        nat_global_set(module)
