...
def adc_get_security_banner(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=system.security_banner.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取登录安全标识失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取登录安全标识失败", response=parsed_data)
            else:
                module.exit_json(changed=False, security_banner=parsed_data)
        except Exception as e:
...
def adc_set_security_banner(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=system.security_banner.set" % (
        ip, authkey)
    ...
    # 添加可选参数
    if 'login_banner' in module.params and module.params['login_banner'] is not None:
        banner_data['login_banner'] = module.params['login_banner']
    if 'login_success_banner' in module.params and module.params['login_success_banner'] is not None:
        banner_data['login_success_banner'] = module.params['login_success_banner']

    # 转换为JSON格式
    post_data = json.dumps(banner_data)
    ...
    try:
        ...
        module.fail_json(msg="设置登录安全标识失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置登录安全标识", True)
        if success:
