...
def adc_list_networks(module):
    ...
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=ospf.network.list" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取OSPF网络列表失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取OSPF网络列表失败", response=parsed_data)
            else:
...
def adc_add_network(module):
    ...
    area = module.params['area'] if 'area' in module.params else ""

    # 检查必需参数
    if not network or not mask or not area:
        module.fail_json(msg="添加OSPF网络需要提供network、mask和area参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=ospf.network.add" % (
        ip, authkey)
...
#         "area": area
    ...
    try:
        ...
        module.fail_json(msg="添加OSPF网络失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "添加OSPF网络", True)
        if success:
...
def adc_delete_network(module):
    ...
    if not network or not mask:
        module.fail_json(msg="删除OSPF网络需要提供network和mask参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=ospf.network.del" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="删除OSPF网络失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "删除OSPF网络", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_get_status(module):
    ...
    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=ospf.status.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取OSPF状态失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取OSPF状态失败", response=parsed_data)
            else:
...
def adc_set_status(module):
    ...
    # 检查必需参数
    if enable == "":
        module.fail_json(msg="设置OSPF状态需要提供enable参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=ospf.status.set" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="设置OSPF状态失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置OSPF状态", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
