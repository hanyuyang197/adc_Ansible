...
def adc_list_maps(module):
    ...
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.map.list" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取NAT映射列表失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取NAT映射列表失败", response=parsed_data)
            else:
                module.exit_json(changed=False, maps=parsed_data)
        except Exception as e:
...
def adc_add_map(module):
    ...
    # 检查必需参数
    if not acl or not nat_pool:
        module.fail_json(msg="添加NAT映射需要提供acl和nat_pool参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.map.add" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="添加NAT映射失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "添加NAT映射", True)
        if success:
...
def adc_delete_map(module):
    ...
    # 检查必需参数
    if not acl or not nat_pool:
        module.fail_json(msg="删除NAT映射需要提供acl和nat_pool参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.map.del" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="删除NAT映射失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "删除NAT映射", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_list_ipv6_maps(module):
    ...
    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.ipv6.map.list" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取IPv6 NAT映射列表失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取IPv6 NAT映射列表失败", response=parsed_data)
            else:
                module.exit_json(changed=False, maps=parsed_data)
        except Exception as e:
            module.fail_json(msg="解析响应失败: %s" % str(e))
...
def adc_add_ipv6_map(module):
    ...
    # 检查必需参数
    if not acl_name or not nat_pool:
        module.fail_json(msg="添加IPv6 NAT映射需要提供acl_name和nat_pool参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.ipv6.map.add" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="添加IPv6 NAT映射失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "添加IPv6 NAT映射", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_delete_ipv6_map(module):
    ...
    # 检查必需参数
    if not acl_name or not nat_pool:
        module.fail_json(msg="删除IPv6 NAT映射需要提供acl_name和nat_pool参数")

    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.ipv6.map.del" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="删除IPv6 NAT映射失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "删除IPv6 NAT映射", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
