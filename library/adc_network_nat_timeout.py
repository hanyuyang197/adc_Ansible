...
def nat_timeout_get(module):
    ...
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=nat.timeout.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取NAT超时配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取NAT超时配置失败", response=parsed_data)
            else:
                module.exit_json(changed=False, timeout=parsed_data)
        except Exception as e:
...
def nat_timeout_set(module):
    ...
    try:
        ...
        module.fail_json(msg="设置NAT超时配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置NAT超时配置", True)
        if success:
