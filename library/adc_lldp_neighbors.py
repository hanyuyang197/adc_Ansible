...
def adc_lldp_neighbors_get(module):
    """LLDP邻居查询"""
    ip = module.params['ip']
    authkey = module.params['authkey']

    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=lldp.neighbors.get" % (ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="LLDP邻居查询失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "LLDP邻居查询", True)
        if success:
