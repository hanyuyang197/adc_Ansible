...
def adc_get_time(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=system.time.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取时间配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取时间配置失败", response=parsed_data)
            else:
                module.exit_json(changed=False, time_config=parsed_data)
        except Exception as e:
...
def adc_set_time(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=system.time.set" % (
        ip, authkey)
    ...
    # 添加可选参数
    if 'date' in module.params and module.params['date'] is not None:
        time_data['date'] = module.params['date']
    if 'time' in module.params and module.params['time'] is not None:
        time_data['time'] = module.params['time']
    if 'tzone' in module.params and module.params['tzone'] is not None:
        time_data['tzone'] = module.params['tzone']

    # 转换为JSON格式
    post_data = json.dumps(time_data)
    ...
    try:
        ...
        module.fail_json(msg="设置时间配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置时间配置", True)
        if success:
...
def main():
    ...
    module_args = dict(
        ...
        tzone=dict(type='str', required=False)
    )
