...
def adc_vrrp_group_del(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=vrrp.group.del" % (
        ip, authkey)
    ...
    optional_params = [
        'group_id', 'description', 'status', 'config', 'setting', 'value', 'enable', 'name', 'ip', 'port'
        # 根据具体API需求添加更多参数
    ]
    ...
    try:
        ...
        module.fail_json(msg="删除指定vrrp组失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "删除指定vrrp组", True)
        if success:
...
def adc_vrrp_group_get(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=vrrp.group.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取指定vrrp组失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "获取指定vrrp组", False)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_vrrp_group_edit(module):
    ...
    if preempt is not None:
        request_data["preempt"] = preempt
    if vrid is not None:
        request_data["vrid"] = vrid
    if failback_first_device is not None:
    ...
    try:
        ...
        module.fail_json(msg="编辑vrrp组失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "编辑vrrp组", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_vrrp_group_stats(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=vrrp.group.stats" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取vrrp组状态列表失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取vrrp组状态列表失败", response=parsed_data)
            else:
...
def main():
    ...
    module_args = dict(
        ...
        failback_first_device=dict(type='int', required=False),
        order_list=dict(type='str', required=False),
