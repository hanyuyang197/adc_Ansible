...
def adc_get_service_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.service.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取业务日志配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取业务日志配置失败", response=parsed_data)
            else:
                module.exit_json(changed=False, service_log_config=parsed_data)
        except Exception as e:
...
def adc_set_service_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.service.set" % (
        ip, authkey)
    ...
    # 添加可选参数
    if 'level' in module.params and module.params['level'] is not None:
        log_data['level'] = module.params['level']
    if 'size' in module.params and module.params['size'] is not None:
        log_data['size'] = module.params['size']

    # 转换为JSON格式
    post_data = json.dumps(log_data)
    ...
    try:
        ...
        module.fail_json(msg="编辑业务日志配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "编辑业务日志配置", True)
        if success:
...
def adc_get_audit_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.audit.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取审计日志配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取审计日志配置失败", response=parsed_data)
            else:
                module.exit_json(changed=False, audit_log_config=parsed_data)
        except Exception as e:
            module.fail_json(msg="解析响应失败: %s" % str(e))
    else:
        module.fail_json(msg="未收到有效响应")
...
def adc_set_audit_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.audit.set" % (
        ip, authkey)
    ...
    if 'disable' in module.params and module.params['disable'] is not None:
        log_data['disable'] = module.params['disable']
    if 'size' in module.params and module.params['size'] is not None:
        log_data['size'] = module.params['size']
    ...
    try:
        ...
        module.fail_json(msg="编辑审计日志配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "编辑审计日志配置", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_get_nat_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.nat.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取NAT日志配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取NAT日志配置失败", response=parsed_data)
            else:
                module.exit_json(changed=False, nat_log_config=parsed_data)
        except Exception as e:
            module.fail_json(msg="解析响应失败: %s" % str(e))
    else:
        module.fail_json(msg="未收到有效响应")
...
def adc_set_nat_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.nat.set" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="编辑NAT日志配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "编辑NAT日志配置", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
...
def adc_get_dns_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.dns.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取DNS日志配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取DNS日志配置失败", response=parsed_data)
            else:
                module.exit_json(changed=False, dns_log_config=parsed_data)
        except Exception as e:
            module.fail_json(msg="解析响应失败: %s" % str(e))
    else:
        module.fail_json(msg="未收到有效响应")
...
def adc_set_dns_log_config(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=log.config.dns.set" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="编辑DNS日志配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "编辑DNS日志配置", True)
        if success:
            module.exit_json(**result_dict)
        else:
            module.fail_json(**result_dict)
