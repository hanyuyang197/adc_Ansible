    ...
    if result:
        try:
            ...
            if 'errmsg' in result and result['errmsg']:
                module.fail_json(msg="获取HTTP2模板列表失败", response=result)
            else:
...
def adc_get_http2_profile(module):
    ...
    if result:
        try:
            # 检查是否有错误信息
            if 'errmsg' in result and result['errmsg']:
                module.fail_json(msg="获取指定HTTP2模板失败", response=result)
            else:
...
def main():
    ...
    if action == 'list_profiles':
        ...
        adc_list_http2_profiles_withcommon(module)
    elif action == 'get_profile':
        adc_get_http2_profile(module)
    elif action == 'add_profile':
        adc_add_http2_profile(module)
    elif action == 'edit_profile':
        adc_edit_http2_profile(module)
