...
def lldp_config_get(module):
    ...
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=lldp.config.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取LLDP配置失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取LLDP配置失败", response=parsed_data)
            else:
...
def lldp_config_set(module):
    ...
    try:
        ...
        module.fail_json(msg="设置LLDP配置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置LLDP配置", True)
        if success:
...
def lldp_neighbors_get(module):
    ...
    # 构造请求URL (使用兼容Python 2.7的字符串格式化)
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=lldp.neighbors.get" % (
        ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="获取LLDP邻居信息失败: %s" % str(e))

    # 对于获取邻居信息操作，直接返回响应数据，不判断success
    if response_data:
        try:
            parsed_data = json.loads(response_data)
            # 检查是否有错误信息
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取LLDP邻居信息失败", response=parsed_data)
            else:
