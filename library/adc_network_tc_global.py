...
def adc_network_tc_global_set(module):
    """流量控制使能全局设置"""
    ip = module.params['ip']
    authkey = module.params['authkey']

    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=network.tc.global.set" % (ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="流量控制使能全局设置失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "流量控制使能全局设置", True)
        if success:
...
def main():
    ...
    # 执行操作
    adc_network_tc_global_set(module)
