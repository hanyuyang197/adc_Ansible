...
def adc_vrrp_force_offline_set(module):
    """设置组vrrp强制下线"""
    ip = module.params['ip']
    ...
    try:
        ...
        module.fail_json(msg="设置组vrrp强制下线失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "设置组vrrp强制下线", True)
        if success:
...
def adc_vrrp_force_offline_get(module):
    """获取组vrrp强制下线状态"""
    ip = module.params['ip']
    authkey = module.params['authkey']
    ...
    try:
        ...
        module.fail_json(msg="获取组vrrp强制下线状态失败: %s" % str(e))
    ...
    if response_data:
        try:
            ...
            if 'errmsg' in parsed_data and parsed_data['errmsg']:
                module.fail_json(msg="获取组vrrp强制下线状态失败", response=parsed_data)
            else:
...
def main():
    ...
    if action == 'set':
        adc_vrrp_force_offline_set(module)
    elif action == 'get':
        adc_vrrp_force_offline_get(module)
