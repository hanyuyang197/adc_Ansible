...
def adc_vrrp_cfgsync_action_pull_push(module):
    ...
    # 构造请求URL
    url = "http://%s/adcapi/v2.0/?authkey=%s&action=vrrp.cfgsync.action.pull_push" % (ip, authkey)
    ...
    try:
        ...
        module.fail_json(msg="配置同步pull-push失败: %s" % str(e))

    # 使用通用响应解析函数
    if response_data:
        success, result_dict = format_adc_response_for_ansible(
            response_data, "配置同步pull-push", True)
        if success:
